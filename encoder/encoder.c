...
#define COPY(var) h->param.var = param->var
    /* COPY( rc.i_qp_constant );  ***ULI***: does not do anything */
...
//      fprintf(stderr, "current: %p  prev: %p  oldest: %p \n", thread_current, thread_prev, thread_oldest);
    ...
    *pp_nal = NULL;
    ...
    if ( rapids_frame_cnt % rapids_qp_steps == 0) {
      printf("Enter qp for current frame %d until frame %d (steps size = %d):\n", 
	     rapids_frame_cnt, rapids_frame_cnt + rapids_qp_steps, rapids_qp_steps);
      
      scanf("%d", &rapids_rate_control);
      printf("\n rapids_rate_control is now %d\n", rapids_rate_control);
    }
...
do_encode:
    ...
    h->fenc->i_qpplus1 = rapids_rate_control + 1; 
    /* h->rc->qpm = rapids_rate_control; /* value returned from x264_ratecontrol_qp( h ) call */
    ...
    i_global_qp = rapids_rate_control; /* RAPIDS */ 
