...
/*****************************************************************************
 * rapids.h  - global "knobs"
 *****************************************************************************/

int   rapids_rate_control = 25; /* dynamic change of quantization within 1..50 range */
int   rapids_qp_steps = 100;    /* granularity of possible qp change */
int   rapids_frame_cnt = 0;     /* counter for current processed frame */
...
#define __PARSEC_XSTRING(x) __PARSEC_STRING(x)
    ...
    /* *** ULI *** sets default values; defined in common/common.c */
    ...
    /* printf("\n*** ULI ***: i_fps_num=%d, i_keyint_max=%d, i_qp_constant=%d\n", param.i_fps_num, param.i_keyint_max,param.rc.i_qp_constant); */
...
#ifdef PTW32_STATIC_LIB
    /* ***ULI*** commenting things out did not help */
    /* ***ULI*** commented out    pthread_win32_thread_detach_np(); */
    /* ***ULI*** commented out   pthread_win32_process_detach_np(); */
#endif

#ifdef ENABLE_PARSEC_HOOKS
    /* ***ULI*** commented out    __parsec_bench_end(); */
#endif
...
#define OPT_LONGHELP 263
        ...
        /* ***ULI*** this routine is defined in directory extras/getopt.c */
    ...
    for( i_frame = 0, i_file = 0; b_ctrl_c == 0 && (i_frame < i_frame_total || i_frame_total == 0); )
    {
      /* printf("\n ***ULI***: Encode frame %d\n\n", i_frame);  ULI */
        ...
        if( opt->qpfile )  {
            ...
            /*  printf("\n ***ULI***: (*1*) Encode frame %d using type %d\n\n", i_frame, pic.i_type); * ULI */
        }
        else
        {
            /* Do not force any parameters */
            pic.i_type = X264_TYPE_AUTO;
            /* pic.i_type = X264_TYPE_I; -> ***ULI*** leads to error message: not compatible with keyframe interval */ 
            pic.i_qpplus1 = 0;
        }

        rapids_frame_cnt = i_frame; /* RAPIDS */
	 
        i_file += Encode_frame( h, opt->hout, &pic );

        /* printf("\n ***ULI***: (*2*) Encoded frame %d using type %d\n\n", i_frame, pic.i_type);  ULI */

        /* ***ULI*** should try insert a call to encoder_reconfig  here */
        /* x264_encoder_reconfig( h, zone->param );  ***ULI*** copied from x264_ratecontrol_start in ratecontrol.c */

        /*
        if (i_frame == 50) {
	  param->rc.i_qp_constant = 20; /* ***ULI***  set to arbitrary value of 20 
          printf("\n ***ULI***: i_qp_constant set is set to %d\n", param->rc.i_qp_constant);
          x264_encoder_reconfig( h, param);  ***ULI*** 
        }
	*/
